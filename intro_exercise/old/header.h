...
/* ΑΠΑΡΑΙΤΗΤΟ ΓΙΑ ΕΓΓΡΑΦΗ ΡΟΥΤΙΝΑΣ ΕΞΥΠΗΡΕΣΗΣ ∆ΙΑΚΟΠΩΝ */ 
int fd2 = open("/proc/FIQ", O_RDWR | O_SYNC); 
/* ΕΝΕΡΓΟΠΟΙΗΣΗ ∆ΙΚΑΙΩΜΑΤΩΝ ΠΡΟΣΠΕΛΑΣΗΣ ΣΤΗΝ*/ /** PIOA & AIC*/ 
char* mptr = mmap(0, 0x1000, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0xFFFFF000); 
/* ΑΠΑΡΑΙΤΗΤΟ ΓΙΑ ΠΡΟΣΠΕΛΑΣΗ ΤΩΝ*/ /**TIMERS*/ 
char* pptr = mmap(0, 0x1000, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0xFFFA0000); 
/* ΕΛΕΓΧΟΣ ΑΠΟΚΤΗΣΗΣ ∆ΙΚΑΙΩΜΑΤΩΝ */ 
if(mptr == MAP_FAILED || pptr == MAP_FAILED){ 
	close(fd); 
	close(fd2); 
	exit(1); 
} 
/* ΟΡΙΖΟΥΜΕ ∆ΙΕΥΘΥΝΣΗ ΤΗΣ*/ /**PIOA*/ 
pioa = (PIO*)(mptr + 0x400); 
/* ΟΡΙΖΟΥΜΕ ∆ΙΕΥΘΥΝΣΗ ΤΗΣ*/ /** AIC*/ 
aic = (AIC*)(mptr); 
/* ΟΡΙΖΟΥΜΕ ∆ΙΕΥΘΥΝΣΗ ΤΟΥ*/ /**TC*/ 
tc = (TC*)(pptr); 
